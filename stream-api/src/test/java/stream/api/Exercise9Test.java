...
public class Exercise9Test extends ClassicOnlineStore {
    ...
    public void mapKeyedByItems() {
        ...
        Supplier<Map<Item, Set<Customer>>> supplier = () -> {
        	return new HashMap<Item, Set<Customer>>();
        };
		BiConsumer<Map<Item, Set<Customer>>, Customer> accumulator = (map, c) -> {

			c.getWantToBuy().stream().forEach(i -> map.computeIfAbsent(i, x -> new HashSet<Customer>()).add(c));

		};
        BinaryOperator<Map<Item, Set<Customer>>> combiner = (map1,map2) -> {
        	map2.forEach(
        		    (key, value) -> map1.merge( key, value, (v1, v2) ->  {v1.addAll(v2);return v1;})
        		);
			return map1;
        };
        
        	
        Function<Map<Item, Set<Customer>>, Map<String, Set<String>>> finisher = m -> 
	        m.entrySet().stream().collect(Collectors.toMap(
					entry -> entry.getKey().getName(), 
					entry -> entry.getValue().stream().map(Customer::getName).collect(Collectors.toSet()))
				);
        ...
        assertThat(itemMap.get("desk"), containsInAnyOrder("Al+ice"));
    }
